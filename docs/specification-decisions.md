...
| Selected behavior | The package preserves absent data, JSON `null`, empty text, empty binary, non-empty binary, and JSON values distinctly. Structured encoding preserves declared JSON payload bytes when the event format can delimit them exactly and materializes `text/plain` only when required to retain text semantics. Binary mappings materialize `application/json`, `text/plain`, or `application/octet-stream` when present data lacks a declared type. Strict encoding fails with `ErrConversionLoss` for every such materialization or explicit type/kind conflict; `Encode*WithReport` exposes the change before returning the mapping. `data` and `data_base64` cannot coexist, and base64 must be canonical padded RFC 4648 encoding. |
...
| Compatibility and wire consequences | Round trips preserve whether data exists, its runtime kind, and which wire member represents it. Loss-aware conversion may add a kind-appropriate content type; strict conversion rejects that metadata change. `text/*` binary payloads decode as text, while other non-JSON types decode as binary. |
...
| Selected behavior | Unknown JSON Boolean and Integer values preserve those abstract types. An unknown JSON string becomes String. Constructors for known extensions may retain Binary, URI, URI-reference, or Timestamp explicitly. Strict encoders reject a target representation that cannot recover that declared type; the corresponding `Encode*WithReport` operation returns the normalized wire value only with an explicit deterministic loss entry. |
...
| Selected behavior | Encoding emits context members in lexicographic order, no package-added insignificant whitespace, UTC RFC 3339 timestamps with only necessary fractional digits, base-10 integers, and padded RFC 4648 base64. Declared JSON payload bytes are embedded unchanged; leading or trailing payload whitespace that a later structured decoder cannot delimit exactly requires `Encode*WithReport` and an explicit loss. This is a Golib byte-stability contract, not CloudEvents or RFC 8785 conformance. |
...
| Executable evidence | `TestEncodeJSONIsDeterministicAndNormative`, `TestStructuredFormatsPreserveDeclaredJSONPayloadBytes`, `TestStrictEncodersRejectEveryImplicitConversionLoss`, and `TestCodecSoakRoundTripsStableBytes` |
| Public surface | `EncodeJSON`, `EncodeJSONWithReport`, JSON batch encoding, structured HTTP encoding, and structured Kafka encoding |
...
| Selected behavior | In binary mode, an empty body with a non-JSON data content type is present empty text or binary data according to that type; with neither bytes nor a content type, data is absent. Strict HTTP encoding rejects present data without a declared content type because an untyped body cannot preserve its runtime kind. `EncodeHTTPWithReport` materializes the kind-appropriate content type and reports that metadata change. A JSON data content type still requires a valid JSON value, so an empty body is invalid. Decoders never close caller-owned readers. Cancellation is checked before and after the bounded read; prompt interruption requires a reader whose own `Read` observes cancellation. No cancellation goroutine is started. |
...
| Executable evidence | `TestDecodeHTTPBinaryRejectsMalformedAttributesAndData`, `TestHTTPBodyLimitsCancellationAndNilOwnership`, `TestHTTPDecodePropagatesBodyFailureWithoutClosingCallerResource`, `TestHTTPDecodeConsumesChunkedShortReads`, and `TestStrictEncodersRejectEveryImplicitConversionLoss` |
| Public surface | `EncodeHTTP`, `EncodeHTTPWithReport`, `DecodeHTTP`, `Limits.MaxEventBytes`, and `Data.Present` |
...
| Selected behavior | Constructing or decoding an event never resolves a schema or performs network I/O. `ValidateSchema` invokes only a caller-supplied `SchemaValidator` with owned event data. Nil contexts, nil validators, and typed-nil validators fail before application code is called. The optional Golib registry adapter requires a bounded cache, an immutable constructor-snapshotted URI-to-lookup mapping, an explicit availability policy, and a positive timeout covering resolution and validation. |
| Security and resource consequences | Core decode has no SSRF, DNS, credential, cache, or remote-resource side effect. The adapter rejects unmapped event-controlled URIs before resolution, so an event cannot select a provider endpoint or registry key. Provider endpoints, credentials, TLS, registry trust, cache limits, and schema compiler policy remain caller-owned. The package preserves cancellation and does not leak aliased payload storage. |
...
| Executable evidence | Core behavior is exercised by `TestValidateSchemaIsExplicitAndOwnsValidatorInput`, `TestValidateSchemaRejectsNilContextPrecisely`, and `TestValidateSchemaRejectsTypedNilValidatorWithoutPanicking`. Adapter-specific mapping, timeout, cancellation, cache, and immutable-snapshot behavior is exercised by `adapters/golib/schema_hardening_test.go` under that independently gated nested module. |
