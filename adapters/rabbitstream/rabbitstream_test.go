...
func TestRabbitStreamStructuredAdapterPreservesEventAndTransportState(t *testing.T) {
	...
	message, err := cloudrabbitstream.Encode(event, cloudrabbitstream.Transport{
		SuperStream: "events", CorrelationID: "correlation-1",
		Properties: []rabbitstream.MetadataEntry{{Key: "transport-attempt", Value: transportProperty}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	transportProperty[0] = 'X'
	if string(message.Properties[0].Value) != "1" {
		t.Fatal("encoded RabbitStream message aliases transport input")
	}
	message.Partition = "events-0"
	message.Stream = "events-0"
	message.PublishingID = 7
	message.HasPublishingID = true
	message.Offset = 42
	message.HasOffset = true

	decoded, state, err := cloudrabbitstream.Decode(message, cloudevents.DefaultLimits())
	if err != nil {
		t.Fatal(err)
	}
	if decoded.Event.ID() != event.ID() || decoded.Event.Type() != event.Type() ||
		state.Stream != "events-0" || state.SuperStream != "events" ||
		state.Partition != "events-0" || state.RoutingKey != "tenant-a" ||
		state.CorrelationID != "correlation-1" || !state.HasPublishingID ||
		state.PublishingID != 7 || !state.HasOffset || state.Offset != 42 ||
		!state.Timestamp.Equal(occurredAt) || len(decoded.TransportProperties) != 1 {
...
func TestRabbitStreamStructuredAdapterRejectsAmbiguousMappings(t *testing.T) {
	...
	for name, transport := range map[string]cloudrabbitstream.Transport{
		"missing target":    {},
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			if _, err := cloudrabbitstream.Encode(event, transport); !errors.Is(err, cloudrabbitstream.ErrMetadataCollision) {
				t.Fatalf("EncodeRabbitStream() error = %v", err)
			}
		})
	}
	if _, err := cloudrabbitstream.Encode(cloudevents.Event{}, cloudrabbitstream.Transport{Stream: "events"}); !errors.Is(err, cloudevents.ErrInvalidEvent) {
		t.Fatalf("invalid event error = %v", err)
	}

	valid, err := cloudrabbitstream.Encode(event, cloudrabbitstream.Transport{SuperStream: "events"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for name, message := range tests {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			if _, _, err := cloudrabbitstream.Decode(message, cloudevents.DefaultLimits()); err == nil {
...
func TestRabbitStreamStructuredAdapterSupportsUnkeyedTimelessEvents(t *testing.T) {
	...
	message, err := cloudrabbitstream.Encode(event, cloudrabbitstream.Transport{Stream: "events"})
	if err != nil {
		t.Fatal(err)
	}
	...
	decoded, _, err := cloudrabbitstream.Decode(message, cloudevents.DefaultLimits())
...
func TestRabbitStreamStructuredAdapterBoundsTransportPropertiesBeforeCopying(t *testing.T) {
	...
	if _, _, err := cloudrabbitstream.Decode(message, cloudevents.DefaultLimits()); !errors.Is(err, rabbitstream.ErrValidation) {
...
func TestRabbitStreamStructuredAdapterBoundsTransportPropertiesBeforeEncoding(t *testing.T) {
	...
	if _, err := cloudrabbitstream.Encode(event, cloudrabbitstream.Transport{
