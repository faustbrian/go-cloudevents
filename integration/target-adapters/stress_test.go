	...
	for range workers {
		...
		go func() {
			...
			for range iterations {
				converted, err := cloudcorrelation.Add(event, values)
				if err != nil {
					failures <- err
					return
				}
				extracted, err := cloudcorrelation.Extract(converted, true, correlation.Policy{})
				if err != nil {
					failures <- err
					return
				}
...
func TestQueueConversionSoakPreservesPayloadAndRetainedState(t *testing.T) {
	...
	state := original
	for range 10_000 {
		event, retained, _, err := cloudqueue.ToCloudEvent(state, cloudqueue.Options{Source: "/queue"})
		if err != nil {
			t.Fatal(err)
		}
		state, _, err = cloudqueue.FromCloudEvent(event, retained)
		if err != nil {
			t.Fatal(err)
		}
	}
