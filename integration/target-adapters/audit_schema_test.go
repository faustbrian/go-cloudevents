...
func TestAuditMetadataAdapterSelectsSafeFieldsAndRequiresTrust(t *testing.T) {
	...
	if _, err := cloudaudit.ExtractMetadata(event, false); !errors.Is(err, cloudevents.ErrUntrustedMetadata) {
...
func TestAuditMetadataRoundTripAllowsAbsentTenant(t *testing.T) {
	...
	event, _, err := cloudaudit.AddMetadata(baseEvent(t), record)
	if err != nil {
		t.Fatal(err)
	}
	metadata, err := cloudaudit.ExtractMetadata(event, true)
...
func TestSchemaAdaptersAreExplicitAndRegistryLookupIsCallerMapped(t *testing.T) {
	...
	direct := cloudjsonschema.Validator{URI: "https://schemas.example/order.json", Schema: compiled}
	for name, validator := range map[string]cloudjsonschema.Validator{
		"empty URI":  {Schema: compiled},
		"nil schema": {URI: "https://schemas.example/order.json"},
	} {
		if err := validator.Validate(ctx, "https://schemas.example/order.json", "application/json", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
	...
	if err := cloudevents.ValidateSchema(ctx, invalid, direct); !errors.Is(err, cloudevents.ErrSchemaViolation) {
	...
	registry := newSchemaHardeningValidator(t, cloudregistry.JSONSchemaConfig{
