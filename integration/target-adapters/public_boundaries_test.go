...
func TestMetadataAdapterRejectsEveryMalformedOrUntrustedBoundary(t *testing.T) {
	...
	if _, err := cloudcorrelation.Add(cloudevents.Event{}, correlation.Values{}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid event error = %v", err)
	}
	if _, err := cloudcorrelation.Add(baseEvent(t), correlation.Values{CorrelationID: correlation.CorrelationID("bad\nvalue")}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid correlation addition error = %v", err)
	}
	values := correlation.Values{CorrelationID: correlation.CorrelationID("same")}
	withSame, err := cloudcorrelation.Add(baseEvent(t), values)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := cloudcorrelation.Add(withSame, values); err != nil {
		t.Fatalf("idempotent correlation addition error = %v", err)
	}

	if extracted, err := cloudcorrelation.Extract(baseEvent(t), true, correlation.Policy{}); err != nil || extracted != (correlation.Values{}) {
	...
	for _, name := range []string{"correlationid", "requestid", "causationid"} {
		...
		if _, err := cloudcorrelation.Extract(event, true, correlation.Policy{}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	if _, err := cloudtenancy.Add(baseEvent(t), tenancy.TenantID{}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid tenant addition error = %v", err)
	}
	if tenant, err := cloudtenancy.Extract(baseEvent(t), true); !errors.Is(err, tenancy.ErrTenantMetadataMissing) || tenant.Valid() {
		t.Fatalf("absent tenant = %v, %v", tenant, err)
	}
	if _, err := cloudtenancy.Extract(eventWithExtensions(t, map[string]cloudevents.Attribute{"tenantid": cloudevents.NewBooleanAttribute(true)}), true); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid tenant kind error = %v", err)
	}
	invalidTenant, _ := cloudevents.NewStringAttribute("bad?tenant")
	if _, err := cloudtenancy.Extract(eventWithExtensions(t, map[string]cloudevents.Attribute{"tenantid": invalidTenant}), true); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
...
func TestTelemetryAdapterCoversNilCollisionBaggageAndTrustedExtraction(t *testing.T) {
	...
	if _, _, err := cloudtelemetry.InjectTraceContext(nilContext, baseEvent(t), policy); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("nil context error = %v", err)
	}
	if _, _, err := cloudtelemetry.InjectTraceContext(context.Background(), baseEvent(t), nil); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("nil policy error = %v", err)
	}
	member, _ := baggage.NewMember("tenant", "a")
	bag, _ := baggage.New(member)
	ctx := baggage.ContextWithBaggage(traceContext(), bag)
	_, report, err := cloudtelemetry.InjectTraceContext(ctx, baseEvent(t), policy)
	...
	if _, _, err := cloudtelemetry.InjectTraceContext(traceContext(), eventWithExtensions(t, map[string]cloudevents.Attribute{"traceparent": existing}), policy); !errors.Is(err, cloudevents.ErrMetadataCollision) {
		t.Fatalf("trace collision error = %v", err)
	}
	if got := cloudtelemetry.ExtractTraceContext(nilContext, baseEvent(t), nil, true); got == nil {
		t.Fatal("nil trace extraction context")
	}
	event, _, err := cloudtelemetry.InjectTraceContext(traceContext(), baseEvent(t), policy)
	if err != nil {
		t.Fatal(err)
	}
	if got := trace.SpanContextFromContext(cloudtelemetry.ExtractTraceContext(context.Background(), event, policy, true)); !got.IsValid() {
...
func TestAuditMetadataRejectsIncompleteMalformedAndCollidingInput(t *testing.T) {
	...
	if _, _, err := cloudaudit.AddMetadata(baseEvent(t), audit.Record{}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	if _, _, err := cloudaudit.AddMetadata(eventWithExtensions(t, map[string]cloudevents.Attribute{"auditid": collision}), record); !errors.Is(err, cloudevents.ErrMetadataCollision) {
		t.Fatalf("audit collision error = %v", err)
	}
	if metadata, err := cloudaudit.ExtractMetadata(baseEvent(t), true); err != nil || metadata != (cloudaudit.Metadata{}) {
		t.Fatalf("absent audit metadata = %#v, %v", metadata, err)
	}
	for _, name := range []string{"auditid", "auditaction", "auditoutcome"} {
		if _, err := cloudaudit.ExtractMetadata(eventWithExtensions(t, map[string]cloudevents.Attribute{name: cloudevents.NewBooleanAttribute(true)}), true); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	for _, outcome := range []string{"bad", "0", "5"} {
		...
		if _, err := cloudaudit.ExtractMetadata(event, true); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	for name, extensions := range map[string]map[string]cloudevents.Attribute{
		...
		if _, err := cloudaudit.ExtractMetadata(eventWithExtensions(t, extensions), true); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	for _, validOutcome := range []string{"1", "2", "3", "4"} {
		...
		if _, err := cloudaudit.ExtractMetadata(event, true); err != nil {
	...
	}), true); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
...
func TestSchemaValidatorsRejectEveryConfigurationResolutionAndPayloadFailure(t *testing.T) {
	...
	for _, validator := range []cloudjsonschema.Validator{{}, {URI: "expected", Schema: compiled}} {
		if err := validator.Validate(ctx, "other", "application/json", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
			t.Fatalf("direct mapping error = %v", err)
		}
	}
	direct := cloudjsonschema.Validator{URI: "schema", Schema: compiled}
	if err := direct.Validate(ctx, "schema", "text/plain", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
	...
	if err := (cloudregistry.JSONSchemaValidator{}).Validate(ctx, "schema", "application/json", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
		t.Fatalf("zero-value registry config error = %v", err)
	}
	if _, err := cloudregistry.NewJSONSchemaValidator(cloudregistry.JSONSchemaConfig{
	...
	}); !errors.Is(err, cloudevents.ErrSchemaMapping) {
	...
	newRegistry := func(mappings map[string]schemaregistry.Lookup) cloudregistry.JSONSchemaValidator {
		return newSchemaHardeningValidator(t, cloudregistry.JSONSchemaConfig{
	...
	if err := registry.Validate(ctx, "schema", "text/plain", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
		t.Fatalf("registry content type error = %v", err)
	}
	if err := registry.Validate(ctx, "other", "application/json", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
	...
	registry = newRegistry(map[string]schemaregistry.Lookup{"schema": lookup})
	if err := registry.Validate(ctx, "schema", "application/json", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaViolation) {
...
func TestKafkaAdapterPropagatesBindingDecodeFailuresAndAcceptsEqualKeys(t *testing.T) {
	...
	if _, err := cloudkafka.Encode(baseEvent(t), cloudevents.BatchMode, cloudkafka.Transport{}); !errors.Is(err, cloudevents.ErrUnsupportedMode) {
	...
	if _, err := cloudkafka.Encode(event, cloudevents.BinaryMode, cloudkafka.Transport{Key: []byte("key")}); err != nil {
		t.Fatalf("equal key error = %v", err)
	}
	if _, _, err := cloudkafka.Decode(kafka.ConsumedRecord{}, cloudevents.Limits{}); !errors.Is(err, cloudevents.ErrLimitExceeded) {
...
func TestKafkaAdapterRejectsOversizedMetadataBeforeCopyingIt(t *testing.T) {
	...
	allocations := testing.AllocsPerRun(100, func() {
		_, _, err := cloudkafka.Decode(record, limits)
	...
	if _, _, err := cloudkafka.Decode(exact, limits); err != nil {
