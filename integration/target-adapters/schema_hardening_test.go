...
func TestRegistryJSONSchemaValidatorRejectsUnmappedEventSchemaWithoutResolution(t *testing.T) {
	...
	validator := newSchemaHardeningValidator(t, cloudregistry.JSONSchemaConfig{
		Cache: cache,
...
func TestRegistryJSONSchemaValidatorRequiresBoundedExplicitConfiguration(t *testing.T) {
	...
	for name, validator := range tests {
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			if _, err := cloudregistry.NewJSONSchemaValidator(validator); !errors.Is(err, cloudevents.ErrSchemaMapping) {
				t.Fatalf("configuration error = %v", err)
			}
		})
	}
	if err := (cloudregistry.JSONSchemaValidator{}).Validate(context.Background(), "schema", "application/json", []byte(`{}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
...
func TestRegistryJSONSchemaValidatorRejectsResolvedNonJSONSchema(t *testing.T) {
	...
	if err := validator.Validate(context.Background(), "schema", "application/json", []byte(`{"id":1}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
...
func TestRegistryJSONSchemaValidatorOwnsImmutableSchemaLookupSnapshot(t *testing.T) {
	...
	if err := validator.Validate(context.Background(), "http://169.254.169.254/latest/meta-data", "application/json", []byte(`{"id":1}`)); !errors.Is(err, cloudevents.ErrSchemaMapping) {
...
func newSchemaHardeningValidator(
	t *testing.T,
	config cloudregistry.JSONSchemaConfig,
) cloudregistry.JSONSchemaValidator {
	t.Helper()
	validator, err := cloudregistry.NewJSONSchemaValidator(config)
	if err != nil {
		t.Fatal(err)
	}
	return validator
