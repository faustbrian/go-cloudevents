...
func TestEventSourcingMappingPreservesCanonicalEnvelopeOutOfBand(t *testing.T) {
	...
	event, state, report, err := cloudeventsourcing.ToCloudEvent(message, cloudeventsourcing.Options{
...
func TestGolibOutboxAndQueueMappingsRetainUnrepresentableState(t *testing.T) {
	...
	queueEvent, queueState, queueReport, err := cloudqueue.ToCloudEvent(queueMessage, cloudqueue.Options{Source: "/queue/jobs"})
	if err != nil || len(queueReport.Losses) != 0 {
		t.Fatalf("queue mapping = %#v, %v", queueReport, err)
	}
	queueRoundTrip, queueReverseReport, err := cloudqueue.FromCloudEvent(queueEvent, queueState)
...
func TestWorkflowMappingRequiresCallerOwnedStableID(t *testing.T) {
	...
	if _, _, _, err := cloudworkflow.ToCloudEvent(history, cloudworkflow.Options{Source: "/workflow"}); err == nil {
