...
func FuzzDecodeKafka(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, value []byte, headerKey string, headerValue []byte) {
		...
		message, _, err := cloudkafka.Decode(record, cloudevents.DefaultLimits())
		if err != nil {
			return
		}
...
func FuzzQueuePayloadRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, payload []byte, tenantValue string) {
		...
		event, state, _, err := cloudqueue.ToCloudEvent(message, cloudqueue.Options{
			Source: "/queue", StableID: "job-1", Type: "example.job",
		})
		if _, tenantErr := tenancy.ParseTenantID(tenantValue); tenantValue != "" && tenantErr != nil {
			if !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
		...
		roundTrip, _, err := cloudqueue.FromCloudEvent(event, state)
		if err != nil {
...
func FuzzEventSourcingRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, payload []byte, tenantValue, metadataValue string) {
		...
		event, state, report, err := cloudeventsourcing.ToCloudEvent(message, cloudeventsourcing.Options{Source: "/event-store"})
		if _, tenantErr := tenancy.ParseTenantID(tenantValue); tenantValue != "" && tenantErr != nil {
			if !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
		...
		roundTrip, reverseReport, err := cloudeventsourcing.FromCloudEvent(event, state)
...
func FuzzOutboxRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, payload []byte, metadataValue string) {
		...
		event, state, report, err := cloudoutbox.ToCloudEvent(envelope, cloudoutbox.Options{
		...
		roundTrip, reverseReport, err := cloudoutbox.FromCloudEvent(event, state)
...
func FuzzWorkflowRoundTrip(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, payload []byte) {
		...
		event, state, report, err := cloudworkflow.ToCloudEvent(history, cloudworkflow.Options{
		...
		roundTrip, reverseReport, err := cloudworkflow.FromCloudEvent(event, state)
...
func FuzzAuditCorrelationTenantMetadata(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, action, correlationValue, causationValue, tenantValue string) {
		...
		converted, report, err := cloudaudit.AddMetadata(fuzzBaseEvent(t), record)
		if _, tenantErr := tenancy.ParseTenantID(tenantValue); tenantValue != "" && tenantErr != nil {
			if !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
		...
		if _, err := cloudaudit.ExtractMetadata(converted, false); !errors.Is(err, cloudevents.ErrUntrustedMetadata) {
		...
		metadata, err := cloudaudit.ExtractMetadata(converted, true)
		if (correlationValue != "" && correlationErr != nil) || (causationValue != "" && causationErr != nil) {
			if !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
...
func FuzzDirectJSONSchemaValidation(f *testing.F) {
	...
	validator := cloudjsonschema.Validator{URI: schemaURI, Schema: compiled}
	...
	f.Fuzz(func(t *testing.T, payload []byte, uri, contentType string) {
		...
		if uri != schemaURI || !fuzzJSONContentType(contentType) {
			if !errors.Is(err, cloudevents.ErrSchemaMapping) {
		...
		case validationErr != nil:
			if err == nil || errors.Is(err, cloudevents.ErrSchemaMapping) || errors.Is(err, cloudevents.ErrSchemaViolation) {
		...
		default:
			if !errors.Is(err, cloudevents.ErrSchemaViolation) {
