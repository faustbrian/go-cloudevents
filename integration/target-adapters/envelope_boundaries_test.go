...
func TestEventSourcingAdapterRejectsEveryUnrepresentableBoundary(t *testing.T) {
	...
	if _, _, _, err := cloudeventsourcing.ToCloudEvent(eventsourcing.Message{}, cloudeventsourcing.Options{Source: "/source"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("zero source message error = %v", err)
	}
	invalidJSON := eventSourcingMessage(t, "application/json", []byte("{"), false)
	if _, _, _, err := cloudeventsourcing.ToCloudEvent(invalidJSON, cloudeventsourcing.Options{Source: "/source"}); err == nil {
	...
	if _, _, _, err := cloudeventsourcing.ToCloudEvent(invalidTenantMessage, cloudeventsourcing.Options{Source: "/source"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
		t.Fatalf("invalid outbound tenant error = %v", err)
	}
	if _, _, _, err := cloudeventsourcing.ToCloudEvent(message, cloudeventsourcing.Options{Source: "bad\nsource"}); err == nil {
		t.Fatal("invalid CloudEvents source error = nil")
	}

	event, state, _, err := cloudeventsourcing.ToCloudEvent(message, cloudeventsourcing.Options{Source: "/source"})
	if err != nil {
		t.Fatal(err)
	}
	if _, _, err := cloudeventsourcing.FromCloudEvent(cloudevents.Event{}, state); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid target event error = %v", err)
	}
	if _, _, err := cloudeventsourcing.FromCloudEvent(eventForEnvelope(t, event.ID(), event.Type(), "other", "application/octet-stream", event.Extensions(), cloudevents.NewBinaryData([]byte("body"))), state); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	if _, _, err := cloudeventsourcing.FromCloudEvent(withoutSchema, state); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	if _, _, err := cloudeventsourcing.FromCloudEvent(withoutSubject, state); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	for _, name := range []string{"correlationid", "causationid", "tenantid", "partitionkey"} {
		...
		t.Run("missing "+name, func(t *testing.T) {
			...
			if _, _, err := cloudeventsourcing.FromCloudEvent(missing, state); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	for _, test := range []struct {
		name   string
		mutate func(*cloudeventsourcing.State)
	}{
		{"correlation", func(value *cloudeventsourcing.State) { value.CorrelationID = "different" }},
		{"causation", func(value *cloudeventsourcing.State) { value.CausationID = "different" }},
		{"tenant", func(value *cloudeventsourcing.State) { value.Tenant = "different" }},
		{"partition", func(value *cloudeventsourcing.State) { value.Partition = "different" }},
	} {
		test := test
		t.Run(test.name, func(t *testing.T) {
			changed := state
			test.mutate(&changed)
			if _, _, err := cloudeventsourcing.FromCloudEvent(event, changed); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	if _, _, err := cloudeventsourcing.FromCloudEvent(withoutData, state); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	if _, _, err := cloudeventsourcing.FromCloudEvent(invalidTenantEvent, invalidTenantState); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
	...
	if _, _, err := cloudeventsourcing.FromCloudEvent(emptyData, state); err == nil {
	...
	if _, _, err := cloudeventsourcing.FromCloudEvent(badID, state); err == nil {
...
func TestOutboxAdapterRejectsEveryInvalidBoundary(t *testing.T) {
	...
	if _, _, _, err := cloudoutbox.ToCloudEvent(outbox.Envelope{}, cloudoutbox.Options{Source: "/source", Type: "type"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid source envelope error = %v", err)
	}
	if _, _, _, err := cloudoutbox.ToCloudEvent(envelope, cloudoutbox.Options{Source: "/source", Type: "type", DataContentType: ";"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid source content type error = %v", err)
	}
	if _, _, _, err := cloudoutbox.ToCloudEvent(envelope, cloudoutbox.Options{Source: "bad\nsource", Type: "type"}); err == nil {
		t.Fatal("invalid source event error = nil")
	}
	if _, _, err := cloudoutbox.FromCloudEvent(cloudevents.Event{}, envelope); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid target event error = %v", err)
	}
	event := eventForEnvelope(t, "event-1", "type", "", "", nil, cloudevents.NewBinaryData([]byte("body")))
	colliding := envelope
	colliding.ID = "other"
	if _, _, err := cloudoutbox.FromCloudEvent(event, colliding); !errors.Is(err, cloudevents.ErrMetadataCollision) {
		t.Fatalf("target ID collision error = %v", err)
	}
	withoutData := eventForEnvelope(t, "event-1", "type", "", "", nil, cloudevents.Data{})
	withoutID := envelope
	withoutID.ID = ""
	if _, _, err := cloudoutbox.FromCloudEvent(withoutData, withoutID); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
...
func TestOutboxAdapterPreservesNilPayload(t *testing.T) {
	...
	event, retained, _, err := cloudoutbox.ToCloudEvent(envelope, cloudoutbox.Options{
		Source: "/outbox", Type: "order.created",
	})
	if err != nil {
		t.Fatal(err)
	}
	roundTrip, _, err := cloudoutbox.FromCloudEvent(event, retained)
	if err != nil {
		t.Fatal(err)
	}
...
func TestQueueAdapterRejectsEveryInvalidBoundary(t *testing.T) {
	...
	if _, _, _, err := cloudqueue.ToCloudEvent(job.Message{}, cloudqueue.Options{Source: "/source", StableID: "id", Type: "type"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid source message error = %v", err)
	}
	if _, _, _, err := cloudqueue.ToCloudEvent(valid, cloudqueue.Options{Source: "/source"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("missing source identity error = %v", err)
	}
	for name, options := range map[string]cloudqueue.Options{
		"missing stable ID": {Source: "/source", Type: "type"},
		"missing type":      {Source: "/source", StableID: "id"},
	} {
		if _, _, _, err := cloudqueue.ToCloudEvent(valid, options); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	if _, _, _, err := cloudqueue.ToCloudEvent(invalidJSON, cloudqueue.Options{Source: "/source"}); err == nil {
	...
	for _, tenantID := range []string{"bad?tenant", " "} {
		...
		if _, _, _, err := cloudqueue.ToCloudEvent(invalidExtension, cloudqueue.Options{Source: "/source"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
	...
	if _, _, _, err := cloudqueue.ToCloudEvent(invalidExtension, cloudqueue.Options{Source: "/source"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid source correlation error = %v", err)
	}
	if _, _, _, err := cloudqueue.ToCloudEvent(valid, cloudqueue.Options{Source: "bad\nsource", StableID: "id", Type: "type"}); err == nil {
		t.Fatal("invalid source event error = nil")
	}

	event := eventForEnvelope(t, "id", "type", "", "", nil, cloudevents.NewBinaryData([]byte("body")))
	if _, _, err := cloudqueue.FromCloudEvent(cloudevents.Event{}, valid); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid target event error = %v", err)
	}
	withoutQueueData := eventForEnvelope(t, "id", "type", "", "", nil, cloudevents.Data{})
	if _, _, err := cloudqueue.FromCloudEvent(withoutQueueData, valid); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("absent target data error = %v", err)
	}
	state, _, err := cloudqueue.FromCloudEvent(event, valid)
	...
	for _, colliding := range []job.Message{
		...
		if _, _, err := cloudqueue.FromCloudEvent(event, colliding); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	if _, _, err := cloudqueue.FromCloudEvent(withTenant, valid); !errors.Is(err, cloudevents.ErrMetadataCollision) {
	...
	if _, _, err := cloudqueue.FromCloudEvent(invalidTenantEvent, valid); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
	...
	if _, _, err := cloudqueue.FromCloudEvent(invalidTenantEvent, invalidTenantState); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) || !errors.Is(err, tenancy.ErrInvalidTenantID) {
...
func TestQueueAdapterPreservesNilBody(t *testing.T) {
	...
	event, retained, _, err := cloudqueue.ToCloudEvent(message, cloudqueue.Options{Source: "/queue"})
	if err != nil {
		t.Fatal(err)
	}
	roundTrip, _, err := cloudqueue.FromCloudEvent(event, retained)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloudEventToQueueRejectsRetainedContentTypeCollision(t *testing.T) {
	...
	if _, _, err := cloudqueue.FromCloudEvent(event, retained); !errors.Is(err, cloudevents.ErrMetadataCollision) {
...
func TestCloudEventToQueueRejectsDeletedRetainedPortableExtensions(t *testing.T) {
	...
	event, retained, _, err := cloudqueue.ToCloudEvent(message, cloudqueue.Options{Source: "/source"})
	if err != nil {
		t.Fatal(err)
	}
	for _, name := range []string{"correlationid", "requestid", "causationid", "traceparent", "tracestate", "tenantid"} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if _, _, err := cloudqueue.FromCloudEvent(missing, retained); !errors.Is(err, cloudevents.ErrMetadataCollision) {
				t.Fatalf("missing retained extension error = %v", err)
			}
		})
	}
...
func TestWorkflowAdapterRejectsEveryInvalidBoundary(t *testing.T) {
	...
	if _, _, _, err := cloudworkflow.ToCloudEvent(history, cloudworkflow.Options{StableID: "id"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("missing workflow source error = %v", err)
	}
	if _, _, _, err := cloudworkflow.ToCloudEvent(workflow.HistoryEvent{}, cloudworkflow.Options{StableID: "id", Source: "/source"}); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("zero workflow history error = %v", err)
	}
	if _, _, _, err := cloudworkflow.ToCloudEvent(history, cloudworkflow.Options{StableID: "id", Source: "bad\nsource"}); err == nil {
		t.Fatal("invalid source event error = nil")
	}
	event, state, _, err := cloudworkflow.ToCloudEvent(history, cloudworkflow.Options{StableID: "id", Source: "/source"})
	if err != nil {
		t.Fatal(err)
	}
	if _, _, err := cloudworkflow.FromCloudEvent(cloudevents.Event{}, state); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
		t.Fatalf("invalid target event error = %v", err)
	}
	for name, changed := range map[string]cloudworkflow.State{
		"missing stable ID": func() cloudworkflow.State { value := state; value.StableID = ""; return value }(),
		"missing sequence":  func() cloudworkflow.State { value := state; value.Sequence = 0; return value }(),
		"different ID":      func() cloudworkflow.State { value := state; value.StableID = "other"; return value }(),
	} {
		if _, _, err := cloudworkflow.FromCloudEvent(event, changed); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
			t.Fatalf("%s target state error = %v", name, err)
		}
	}
	wrongType := eventForEnvelope(t, event.ID(), "other", eventSubject(event), "", nil, event.Data())
	if _, _, err := cloudworkflow.FromCloudEvent(wrongType, state); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	if _, _, err := cloudworkflow.FromCloudEvent(withoutPortableFields, state); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	for name, candidate := range map[string]cloudevents.Event{
		...
		if _, _, err := cloudworkflow.FromCloudEvent(candidate, state); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
	...
	if _, _, err := cloudworkflow.FromCloudEvent(invalidKind, state); err == nil {
...
func TestWorkflowAdapterPreservesNilData(t *testing.T) {
	...
	event, retained, _, err := cloudworkflow.ToCloudEvent(history, cloudworkflow.Options{
	...
	roundTrip, _, err := cloudworkflow.FromCloudEvent(event, retained)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkflowAdapterRejectsNonCanonicalEventTypes(t *testing.T) {
	...
	event, retained, _, err := cloudworkflow.ToCloudEvent(workflowHistory(t), cloudworkflow.Options{
		StableID: "workflow-event", Source: "/workflow",
	})
	if err != nil {
		t.Fatal(err)
	...
	for _, eventType := range []string{
		...
		if _, _, err := cloudworkflow.FromCloudEvent(candidate, retained); !errors.Is(err, cloudevents.ErrInvalidAdapterInput) {
...
func TestWorkflowAdapterReportsEveryDiscardedPayloadDeclaration(t *testing.T) {
	...
	_, state, _, err := cloudworkflow.ToCloudEvent(workflowHistory(t), cloudworkflow.Options{
		StableID: "workflow-event", Source: "/workflow",
	})
	if err != nil {
		t.Fatal(err)
	...
	if _, report, err := cloudworkflow.FromCloudEvent(event, state); err != nil {
		t.Fatal(err)
	} else {
...
func TestEnvelopeAdaptersReportEveryPayloadKindTheyCannotReconstruct(t *testing.T) {
	...
	assertDataKindLoss := func(t *testing.T, report cloudevents.AdapterReport, err error) {
		t.Helper()
		if err != nil {
			t.Fatal(err)
		}
	...
	message := eventSourcingMessage(t, "application/octet-stream", []byte("body"), true)
	eventSourcingEvent, eventState, _, err := cloudeventsourcing.ToCloudEvent(message, cloudeventsourcing.Options{Source: "/source"})
	if err != nil {
		t.Fatal(err)
	}
	event := eventForEnvelope(
	...
	_, report, err := cloudeventsourcing.FromCloudEvent(event, eventState)
	...
	_, report, err = cloudoutbox.FromCloudEvent(event, outboxState)
	assertDataKindLoss(t, report, err)

	queueEvent, queueState, _, err := cloudqueue.ToCloudEvent(
		job.Message{Timeout: time.Second, Body: []byte("body")},
		cloudqueue.Options{Source: "/source", StableID: "job-1", Type: "job.type"},
	)
	if err != nil {
		t.Fatal(err)
	}
	event = eventForEnvelope(t, queueEvent.ID(), queueEvent.Type(), "", "", queueEvent.Extensions(), jsonData)
	_, report, err = cloudqueue.FromCloudEvent(event, queueState)
	assertDataKindLoss(t, report, err)
}
